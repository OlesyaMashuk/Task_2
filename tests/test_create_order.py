...
class TestCreateOrder:
    ...
    def test_create_order_not_auth(self):
        r = requests.post(f"{Curls.MAIN_URL}{Curls.URL_CREATE_ORDER}", data=Ingredients.valid_ingredients_data)
        assert r.status_code == 200
        assert r.json().get("success") is True
    ...
    def test_create_order_with_ingridient(self):
        r = requests.post(f"{Curls.MAIN_URL}{Curls.URL_CREATE_ORDER}")
    ...
    def test_create_order_invalid_hash_ingridient(self):
        response = requests.post(Curls.MAIN_URL + Curls.URL_CREATE_ORDER, headers=Curls.headers,
